...
export const PageSettings: React.FC<PageSettingsProps> = ({ addNotification }) => {
 ...
 <div>
 <h3 className="text-base font-black text-[var(--text-primary)] dark:text-[var(--text-primary)] uppercase tracking-wider">Halaman Statis (CMS)</h3>
 <p className="text-xs text-[var(--text-secondary)] dark:text-[var(--text-secondary)] dark:text-[var(--text-secondary)] mt-1">Kelola konten publik dengan Live Markdown Editor</p>
 </div>
 </div>
 <button
 type="button"
 ...
 {/* Tabs */}
 <div className="flex gap-2 border-b border-[var(--glass-border)] dark:border-white/[0.06] pb-4 mb-6 overflow-x-auto scrollbar-hide">
 <button
 ...
 {/* Editor Side */}
 <div className="flex flex-col border border-[var(--glass-border)] dark:border-white/[0.06] rounded-xl overflow-hidden bg-white dark:bg-red-950">
 <div className=" border-b border-[var(--glass-border)] dark:border-white/[0.06] p-2 flex items-center gap-1">
 <button onClick={() => insertFormat('**', '**')} className="p-1.5 hover:bg-slate-200 dark:hover:bg-[var(--bg-secondary)] rounded text-slate-600 dark:text-[var(--text-secondary)] dark:text-[var(--text-secondary)]" title="Bold"><Bold className="w-4 h-4" /></button>
 <button onClick={() => insertFormat('*', '*')} className="p-1.5 hover:bg-slate-200 dark:hover:bg-[var(--bg-secondary)] rounded text-slate-600 dark:text-[var(--text-secondary)] dark:text-[var(--text-secondary)]" title="Italic"><Italic className="w-4 h-4" /></button>
 <div className="w-px h-4 bg-slate-300 dark:bg-slate-700 mx-1" />
 <button onClick={() => insertFormat('## ')} className="p-1.5 hover:bg-slate-200 dark:hover:bg-[var(--bg-secondary)] rounded text-slate-600 dark:text-[var(--text-secondary)] dark:text-[var(--text-secondary)]" title="Heading 2"><Heading2 className="w-4 h-4" /></button>
 <button onClick={() => insertFormat('- ')} className="p-1.5 hover:bg-slate-200 dark:hover:bg-[var(--bg-secondary)] rounded text-slate-600 dark:text-[var(--text-secondary)] dark:text-[var(--text-secondary)]" title="List"><List className="w-4 h-4" /></button>
 <button onClick={() => insertFormat('[', '](url)')} className="p-1.5 hover:bg-slate-200 dark:hover:bg-[var(--bg-secondary)] rounded text-slate-600 dark:text-[var(--text-secondary)] dark:text-[var(--text-secondary)]" title="Link"><LinkIcon className="w-4 h-4" /></button>
 <div className="ml-auto px-3 text-[10px] font-bold uppercase tracking-wider text-[var(--text-secondary)] dark:text-[var(--text-secondary)]">Markdown Editor</div>
 </div>
 <textarea
 ref={textareaRef}
 value={pagesConfig[activeTab]}
 onChange={(e) => setPagesConfig(prev => ({ ...prev, [activeTab]: e.target.value }))}
 className="flex-1 w-full bg-transparent p-4 text-sm text-[var(--text-primary)] dark:text-slate-200 font-mono resize-none focus:outline-none"
 ...
 {/* Live Preview Side */}
 <div className="flex flex-col border border-[var(--glass-border)] dark:border-white/[0.06] rounded-xl overflow-hidden bg-white dark:bg-red-950">
 <div className=" border-b border-[var(--glass-border)] dark:border-white/[0.06] p-3 flex justify-between items-center">
 <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-secondary)] dark:text-[var(--text-secondary)]">Live Preview</span>
