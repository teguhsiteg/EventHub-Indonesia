...
export const PaymentActionModal: React.FC<PaymentActionModalProps> = ({
 ...
 return (
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--bg-primary)]/50 backdrop-blur-sm animate-in fade-in duration-200">
 <div className="bg-white dark:bg-[var(--bg-primary)] w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-[var(--glass-border)] ">
 ...
 <p className="text-xs text-[var(--text-secondary)] font-mono mt-0.5">{payment.invoiceId}</p>
 </div>
 </div>
 <button 
 onClick={onClose}
 className="p-2 text-[var(--text-secondary)] hover:text-slate-600 dark:hover:text-slate-300 hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-secondary)] rounded-lg transition-colors"
 >
 <X className="w-5 h-5" />
 </button>
 </div>
 ...
 {/* Participant Info */}
 <div className="bg-slate-50 dark:bg-[var(--bg-secondary)]/50 p-4 rounded-xl border border-slate-100 ">
 <div className="grid grid-cols-2 gap-4 text-sm">
 <div>
 <p className="text-[10px] text-[var(--text-secondary)] font-bold uppercase mb-1">Nama Peserta</p>
 <p className="font-semibold text-[var(--text-primary)] ">{participantName}</p>
 </div>
 <div>
 <p className="text-[10px] text-[var(--text-secondary)] font-bold uppercase mb-1">No. Registrasi</p>
 <p className="font-mono font-medium text-slate-700 ">{registration?.registrationNumber || '-'}</p>
 </div>
 </div>
 </div>
 ...
 <div>
 <p className="text-[10px] text-[var(--text-secondary)] font-bold uppercase mb-1">Nominal</p>
 <p className="font-black text-emerald-600 dark:text-emerald-400">{formatRupiah(payment.amount)}</p>
 </div>
 <div>
 <p className="text-[10px] text-[var(--text-secondary)] font-bold uppercase mb-1">Metode Pembayaran</p>
 <p className="font-semibold text-slate-700 ">{payment.paymentMethod}</p>
 </div>
 <div>
 <p className="text-[10px] text-[var(--text-secondary)] font-bold uppercase mb-1">Status</p>
 ...
 'bg-[var(--bg-secondary)] text-slate-700 border-[var(--glass-border)] dark:bg-[var(--bg-secondary)] dark:text-[var(--text-secondary)] '
 }`}>
 {payment.status}
 </span>
 </div>
 <div>
 <p className="text-[10px] text-[var(--text-secondary)] font-bold uppercase mb-1">Tanggal Dibuat</p>
 <p className="text-xs text-slate-700 ">{formatDate(payment.createdAt)}</p>
 </div>
 </div>

 {payment.proofUrl && (
 <div className="pt-4 border-t border-[var(--glass-border)] ">
 <p className="text-[10px] text-[var(--text-secondary)] font-bold uppercase mb-2">Bukti Pembayaran</p>
 <a 
 ...
 <div>
 <label className="block text-[10px] text-[var(--text-secondary)] font-bold uppercase mb-1.5">
 ...
 className="w-full bg-white dark:bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-lg px-4 py-2.5 text-sm font-semibold text-[var(--text-primary)] focus:ring-2 focus:ring-red-500 focus:border-red-500"
 />
 <p className="text-[10px] text-[var(--text-secondary)] mt-1">Ubah jika terdapat ketidaksesuaian nominal transfer.</p>
 </div>
 ...
 className="w-full bg-white dark:bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-lg px-4 py-2.5 text-sm font-semibold text-[var(--text-primary)] focus:ring-2 focus:ring-red-500 focus:border-red-500 appearance-none"
 >
 ...
 {/* Footer */}
 <div className="p-4 sm:p-6 border-t border-[var(--glass-border)] bg-slate-50 dark:bg-[var(--bg-primary)]/50 flex justify-end gap-3">
 <button
 onClick={onClose}
 className="px-5 py-2.5 rounded-xl text-xs font-bold text-slate-600 dark:text-[var(--text-secondary)] hover:bg-slate-200 dark:hover:bg-[var(--bg-secondary)] transition-colors"
 >
 Tutup
 </button>
 {mode === 'EDIT' && (
