      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-4xl w-full shadow-2xl overflow-hidden my-8">
        ...
        <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-950/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-black text-white uppercase tracking-wider">E-Sertifikat Finisher Resmi</h3>
              <p className="text-xs text-slate-400">Nomor Sertifikat: {certificate.certificateNumber}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-8 bg-slate-950 overflow-x-auto flex justify-center">
          <div
            ref={printRef}
          ...
          >
            ...
            <div className="absolute top-6 left-8 flex items-center gap-2">
              ...
              <span className="text-sm font-black text-white tracking-widest uppercase">RACEPRO</span>
            </div>
            ...
            <div className="mt-8 space-y-2">
              ...
              <h2 className="text-3xl font-black text-white tracking-wide font-serif italic">
            ...
            <div className="my-6 space-y-3">
              <p className="text-xs text-slate-400 uppercase tracking-widest">Diberikan Dengan Bangga Kepada:</p>
              ...
              <p className="text-xs text-slate-300 max-w-lg mx-auto leading-relaxed pt-2">
                Atas keberhasilan menuntaskan lomba dalam kategori <strong className="text-white">{certificate.categoryName}</strong> dengan nomor peserta BIB <strong className="text-amber-400">{certificate.bibNumber}</strong>.
              </p>
            </div>

            {/* Metrics */}
            <div className="grid grid-cols-3 gap-4 max-w-xl mx-auto my-2 bg-slate-900/80 border border-amber-500/20 p-4 rounded-xl">
              <div>
                <span className="block text-[10px] font-bold text-slate-400 uppercase">WAKTU FINISH</span>
                <span className="block text-xl font-black text-white font-mono mt-0.5">{certificate.finishTime}</span>
              </div>
              <div>
                <span className="block text-[10px] font-bold text-slate-400 uppercase">RANK KATEGORI</span>
                <span className="block text-xl font-black text-amber-400 font-mono mt-0.5">#{certificate.categoryRank}</span>
              </div>
              <div>
                <span className="block text-[10px] font-bold text-slate-400 uppercase">RANK OVERALL</span>
                <span className="block text-xl font-black text-white font-mono mt-0.5">#{certificate.rank}</span>
              </div>
            </div>

            {/* Footer Signatures */}
            <div className="flex items-end justify-between text-left text-[10px] text-slate-400 pt-4 border-t border-slate-800">
              <div>
                <p className="font-semibold text-slate-300">Sistem Verifikasi RacePro</p>
              ...
              <div className="text-right">
                <div className="w-24 h-12 border-b border-slate-700 mb-1 flex items-center justify-end">
                  <Sparkles className="w-6 h-6 text-amber-400/50" />
                </div>
                <p className="font-bold text-slate-200 uppercase">Race Director</p>
        ...
        <div className="p-6 bg-slate-950/80 border-t border-slate-800 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={handlePrint}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs uppercase tracking-wider transition-all"
            >
