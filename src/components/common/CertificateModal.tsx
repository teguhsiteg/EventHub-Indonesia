...
export const CertificateModal: React.FC<CertificateModalProps> = ({ certificate, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950/80 backdrop-blur-md overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-4xl w-full shadow-2xl overflow-hidden my-8">
        ...
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-950/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-wider">E-Sertifikat Finisher Resmi</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Nomor Sertifikat: {certificate.certificateNumber}</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white rounded-lg hover:bg-slate-100 dark:bg-slate-800 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-8 bg-slate-50 dark:bg-slate-950 overflow-x-auto flex justify-center">
          <div
            ref={printRef}
          ...
          >
            ...
            <div className="my-6 space-y-3">
              <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-widest">Diberikan Dengan Bangga Kepada:</p>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 max-w-lg mx-auto leading-relaxed pt-2">
            ...
            {/* Metrics */}
            <div className="grid grid-cols-3 gap-4 max-w-xl mx-auto my-2 bg-white dark:bg-slate-900/80 border border-amber-500/20 p-4 rounded-xl">
              <div>
                <span className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">WAKTU FINISH</span>
                <span className="block text-xl font-black text-slate-900 dark:text-white font-mono mt-0.5">{certificate.finishTime}</span>
              </div>
              <div>
                <span className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">RANK KATEGORI</span>
                <span className="block text-xl font-black text-amber-400 font-mono mt-0.5">#{certificate.categoryRank}</span>
              </div>
              <div>
                <span className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">RANK OVERALL</span>
                <span className="block text-xl font-black text-slate-900 dark:text-white font-mono mt-0.5">#{certificate.rank}</span>
              </div>
            </div>

            {/* Footer Signatures */}
            <div className="flex items-end justify-between text-left text-[10px] text-slate-500 dark:text-slate-400 pt-4 border-t border-slate-200 dark:border-slate-800">
              <div>
                <p className="font-semibold text-slate-600 dark:text-slate-300">Sistem Verifikasi RacePro</p>
        ...
        <div className="p-6 bg-slate-50 dark:bg-slate-950/80 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={handlePrint}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-700 text-slate-900 dark:text-white font-bold text-xs uppercase tracking-wider transition-all"
            >
