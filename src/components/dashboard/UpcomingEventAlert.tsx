...
export const UpcomingEventAlert: React.FC<UpcomingEventAlertProps> = ({
 ...
 <div>
 <h2 className="text-xl sm:text-2xl font-black text-[var(--text-primary)] dark:text-[var(--text-primary)] uppercase tracking-tight">
 {event.name}
 </h2>
 <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-600 mt-1 flex items-center gap-1.5 font-medium">
 ...
 : 'bg-white dark:bg-red-950/90 border-[var(--glass-border)] text-slate-600 dark:text-slate-600 '
 }`}>
 <CheckCircle2 className={`w-3.5 h-3.5 ${participant?.checkInStatus ? 'text-emerald-400' : 'text-[var(--text-secondary)]'}`} />
 ...
 className="px-5 py-2.5 rounded-xl bg-white dark:bg-red-950/90 hover:bg-[var(--bg-secondary)] dark:bg-[var(--bg-secondary)] border border-yellow-500/50 text-yellow-300 font-bold text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-2"
 >
