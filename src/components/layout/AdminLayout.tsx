...
export const AdminLayout: React.FC<AdminLayoutProps> = ({ children }) => {
 ...
 return (
 <div className="relative min-h-screen font-sans text-[var(--text-primary)] bg-slate-50/50 selection:bg-red-100 selection:text-red-900">
 {/* Background */}
 ...
 className="xl:hidden p-2 rounded-lg hover:bg-[var(--bg-primary)] text-[var(--text-secondary)]"
 >
 ...
 <Link to="/" className="p-2 rounded-lg hover:bg-[var(--bg-primary)] text-[var(--text-secondary)] hover:text-slate-600 transition-colors" title="Ke Website">
 ...
 : 'text-[var(--text-secondary)] hover:bg-[var(--bg-primary)] hover:text-slate-700'
 }`}
 >
 <Icon className={`w-4 h-4 ${isActive ? 'text-red-500' : 'text-[var(--text-secondary)]'}`} />
 <span>{tab.label}</span>
 </Link>
 );
 })}
 ...
 <aside className="absolute top-16 left-0 bottom-0 w-64 bg-slate-50 dark:bg-red-950 border-r border-[var(--glass-border)] dark:border-white/[0.06] p-3 flex flex-col gap-1 animate-in slide-in-from-left">
 {(isSuperAdmin ? adminTabs : adminTabs.filter(tab => !['requests', 'users', 'settings'].includes(tab.id))).map(tab => {
 const Icon = tab.icon;
 const tabParam = new URLSearchParams(tab.href.includes('?') ? tab.href.split('?')[1] : '').get('tab');
 const currentParam = location.search ? new URLSearchParams(location.search).get('tab') : null;
 ...
 : 'text-slate-600 dark:text-[var(--text-secondary)] hover:text-[var(--text-primary)] dark:hover:text-white hover:bg-slate-200/50 dark:hover:bg-white/[0.04]'
 }`}
 >
 ...
 <div className="border-t border-[var(--glass-border)] dark:border-white/[0.06] mt-2 pt-2">
 <Link
 to="/admin/check-in"
 onClick={() => setMobileSidebarOpen(false)}
