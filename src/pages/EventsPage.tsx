    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-900 border border-slate-800 text-orange-400 text-[10px] font-black uppercase tracking-wider mb-2">
              <Sparkles className="w-3 h-3 text-amber-400" />
              <span>GUWIGO EVENTS FIRESTORE SEARCH</span>
            </div>
            <h1 className="text-3xl font-black text-white uppercase tracking-tight">Jelajah Event Olahraga</h1>
        ...
        {loading || isSearching ? (
          ...
          <div className="p-16 text-center bg-slate-900/50 rounded-2xl border border-slate-800 max-w-lg mx-auto">
            <Trophy className="w-12 h-12 text-slate-600 mx-auto mb-4" />
            <h3 className="text-lg font-bold text-white">Event Tidak Ditemukan</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {events.map(event => (
              <div key={event.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden flex flex-col hover:border-slate-700 transition-all group shadow-lg">
                <div className="relative h-48 bg-slate-950 overflow-hidden">
                  ...
                  {event.category && (
                    <div className="absolute top-3 right-3 bg-slate-950/80 backdrop-blur-md border border-slate-800 text-amber-400 text-[10px] font-bold uppercase px-2.5 py-1 rounded flex items-center gap-1">
                ...
                <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                  <div>
                    ...
                    <h3 className="text-lg font-bold text-white line-clamp-1 group-hover:text-orange-400 transition-colors">{event.name}</h3>
                    <p className="text-xs text-slate-400 mt-1.5 line-clamp-2 leading-relaxed">{event.description}</p>
