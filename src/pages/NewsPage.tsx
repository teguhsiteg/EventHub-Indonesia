...
export const NewsPage: React.FC = () => {
 ...
 return (
 <div className="min-h-screen bg-white dark:bg-[#0B0F14] text-[var(--text-primary)] dark:text-gray-200 pb-16">
 {/* Breadcrumb */}
 ...
 <span className="text-[var(--accent)] dark:text-amber-400">Berita</span>
 </nav>
 </div>
 ...
 <Megaphone className="w-4 h-4 text-[var(--accent)] dark:text-amber-400" />
 <span>Guwigo Events — BERITA & PENGUMUMAN</span>
 </div>
 <h1 className="text-4xl sm:text-5xl md:text-6xl font-black text-[var(--text-primary)] uppercase tracking-tight leading-[1.05]">
 Berita & <br />
 <span className="text-transparent bg-clip-text bg-gradient-to-r from-[var(--accent)] to-amber-400">
 Pengumuman
 </span>
 </h1>
 <p className="text-slate-600 dark:text-[var(--text-secondary)] text-base mt-4 max-w-2xl leading-relaxed">
 ...
 {[1,2,3].map(i => <div key={i} className="h-32 rounded-2xl bg-[var(--bg-secondary)] dark:bg-gray-800 animate-pulse" />)}
 </div>
 ) : announcements.length === 0 ? (
 <div className="text-center py-16 bg-white dark:bg-gray-900 border border-[var(--glass-border)] dark:border-gray-800 rounded-2xl">
 <Megaphone className="w-12 h-12 text-slate-300 mx-auto mb-3" />
 <h3 className="text-lg font-bold text-[var(--text-primary)] ">Belum ada berita</h3>
 <p className="text-sm text-[var(--text-secondary)] mt-1">Pantau terus halaman ini untuk update terbaru.</p>
 </div>
 ) : (
 <div className="space-y-4">
 {announcements.map(item => (
 <article key={item.id} className={`bg-white dark:bg-gray-900 border rounded-2xl p-6 transition-all hover:shadow-md ${
 item.isPinned ? 'border-amber-300 dark:border-amber-800 shadow-md' : 'border-[var(--glass-border)] dark:border-gray-800'
 }`}>
 <div className="flex items-start gap-4">
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2 mb-2">
 {item.isPinned && <Pin className="w-4 h-4 text-[var(--accent)] shrink-0" />}
 <h2 className="text-lg font-bold text-[var(--text-primary)] ">{item.title}</h2>
 </div>
 <p className="text-sm text-slate-600 dark:text-[var(--text-secondary)] leading-relaxed whitespace-pre-line">{item.content}</p>
 <div className="flex items-center gap-1.5 mt-4 text-xs text-[var(--text-secondary)]">
