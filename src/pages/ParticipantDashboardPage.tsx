...
export const ParticipantDashboardPage: React.FC = () => {
 ...
 return (
 <div className="w-full max-w-5xl mx-auto text-[var(--text-primary)] dark:text-slate-100 animate-in fade-in duration-500">
 ...
 {/* Minimal Avatar */}
 <div className="w-16 h-16 rounded-full bg-gradient-to-br from-amber-500 to-amber-500 text-white shadow-lg flex items-center justify-center shrink-0 border-2 border-white ">
 <User className="w-8 h-8" />
 </div>
 
 <div>
 <div className="flex items-center gap-2 mb-1">
 <span className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest bg-[var(--bg-secondary)] dark:bg-[var(--bg-secondary)] px-2 py-0.5 rounded-md">
 Peserta
 </span>
 ...
 <h1 className="text-2xl font-bold text-[var(--text-primary)] tracking-tight">{user?.displayName}</h1>
 ...
 className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-[var(--bg-primary)] hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-secondary)] dark:text-[var(--text-primary)] text-white font-semibold text-sm transition-all duration-300 hover:scale-[1.02] active:scale-95 shadow-lg shadow-slate-900/20"
 >
 ...
 className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-white dark:bg-[var(--bg-secondary)] hover:bg-[var(--bg-main)] dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold text-sm border border-[var(--glass-border)] transition-all duration-300 hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:hover:scale-100 shadow-sm"
 >
 ...
 <div className="mb-8">
 <h3 className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider mb-3">Pilih Pendaftaran:</h3>
 <div className="inline-flex items-center p-1 bg-[var(--bg-secondary)] dark:bg-[var(--bg-secondary)] rounded-2xl overflow-x-auto custom-scrollbar">
 {registrations.map(r => (
 <button
 key={r.id}
 onClick={() => {
 ...
 selectedReg?.id === r.id 
 ? 'bg-white dark:bg-slate-700 text-[var(--text-primary)] shadow-sm scale-100' 
 : 'text-[var(--text-secondary)] hover:text-slate-700 dark:text-[var(--text-secondary)] dark:hover:text-slate-200 scale-95 hover:scale-100'
 }`}
 >
 ...
 {/* Tab Contents */}
 {loading ? (
 <div className="bg-white dark:bg-[var(--bg-primary)] h-64 rounded-2xl animate-pulse border border-[var(--glass-border)] " />
 ) : registrations.length === 0 ? (
 <div className="p-12 sm:p-20 text-center glass-card">
 <div className="w-16 h-16 mb-4 rounded-full bg-[var(--bg-main)] dark:bg-[var(--bg-secondary)] flex items-center justify-center mx-auto">
 <FileText className="w-6 h-6 text-[var(--text-secondary)]" />
 </div>
 <h3 className="text-xl font-bold text-[var(--text-primary)] tracking-tight mb-2">Belum Ada Pendaftaran</h3>
 ...
 <button onClick={() => navigate('/events')} className="px-8 py-3 bg-[var(--bg-primary)] hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-secondary)] text-white dark:text-[var(--text-primary)] text-sm font-semibold rounded-xl shadow-lg transition-all duration-300 hover:scale-[1.02] active:scale-95">
 Lihat Katalog Event
 </button>
 </div>
 ) : (
 ...
 <div>
 <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">Nomor Registrasi</p>
 <h3 className="text-lg font-mono font-semibold text-[var(--text-primary)] ">{selectedReg.registrationNumber}</h3>
 </div>
 ...
 'bg-[var(--bg-main)] text-slate-700 dark:bg-[var(--bg-secondary)] border border-[var(--glass-border)] '
 }`}>
 ...
 <div className="bg-[var(--bg-main)] dark:bg-[var(--bg-secondary)]/50 p-4 rounded-xl border border-slate-100 text-center">
 <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">No. BIB</p>
 <p className="text-2xl font-display tracking-widest text-[var(--accent)] dark:text-[var(--accent)]">{participant?.bibNumber || '-'}</p>
 </div>
 <div className="bg-[var(--bg-main)] dark:bg-[var(--bg-secondary)]/50 p-4 rounded-xl border border-slate-100 text-center">
 <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">Jersey</p>
 <p className="text-lg font-mono font-bold text-[var(--text-primary)] ">{participant?.jerseySize || '-'}</p>
 </div>
 <div className="bg-[var(--bg-main)] dark:bg-[var(--bg-secondary)]/50 p-4 rounded-xl border border-slate-100 text-center flex flex-col items-center justify-center">
 <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">Race Pack</p>
 {participant?.checkInStatus ? (
 <CheckCircle2 className="w-5 h-5 text-emerald-500 mt-0.5" />
 ) : (
 <p className="text-xs font-semibold text-[var(--text-secondary)] mt-1">Pending</p>
 )}
 </div>
 </div>
 </div>
 ...
 <img src={eventData.banner} alt={eventData.name} className="w-20 h-20 rounded-xl object-cover shrink-0 border border-slate-100 " />
 <div>
 <h4 className="text-base font-bold text-[var(--text-primary)] ">{eventData.name}</h4>
 <p className="text-sm text-[var(--text-secondary)] mt-1">
 {new Date(eventData.startDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
 </p>
 <p className="text-xs text-[var(--text-secondary)] mt-0.5">{eventData.location}</p>
 </div>
 </div>
 )}
 </div>
 ...
 <div className="mb-4">
 <QrCode className="w-8 h-8 text-[var(--text-secondary)] mx-auto" />
 </div>
 <h3 className="text-sm font-bold text-[var(--text-primary)] mb-1">Identitas Peserta</h3>
 <p className="text-xs text-[var(--text-secondary)] mb-6">Gunakan QR Code ini saat pengambilan Race Pack.</p>
 
 {participant ? (
 <div className="p-3 bg-white border border-[var(--glass-border)] rounded-xl">
 <QRCodeViewer value={participant.qrToken} label="" />
 </div>
 ) : (
 <div className="w-48 h-48 bg-[var(--bg-main)] dark:bg-[var(--bg-secondary)] rounded-xl border-2 border-dashed border-[var(--glass-border)] flex items-center justify-center">
 <p className="text-xs text-[var(--text-secondary)]">QR Belum Tersedia</p>
 </div>
 )}
 </div>
 </div>
 ...
 {/* Receipt Header */}
 <div className="bg-[var(--bg-main)] dark:bg-[var(--bg-secondary)]/50 p-6 border-b border-[var(--glass-border)] flex items-center justify-between">
 <div>
 <h3 className="text-lg font-bold text-[var(--text-primary)] ">Rincian Pembayaran</h3>
 <p className="text-xs text-[var(--text-secondary)] font-mono mt-1">INV: {payment.invoiceId}</p>
 </div>
 ...
 <div className="flex justify-between items-center py-2 border-b border-dashed border-[var(--glass-border)] ">
 <span className="text-sm text-slate-600 dark:text-[var(--text-secondary)]">Total Tagihan</span>
 <span className="text-lg font-bold text-[var(--text-primary)] ">{formatRupiah(payment.amount)}</span>
 </div>
 <div className="flex justify-between items-center py-2">
 <span className="text-sm text-slate-600 dark:text-[var(--text-secondary)]">Metode</span>
 <span className="text-sm font-semibold text-[var(--text-primary)] ">{payment.paymentMethod}</span>
 </div>
 </div>

 {/* Action Area */}
 <div className="p-6 bg-[var(--bg-main)] dark:bg-[var(--bg-secondary)]/30 border-t border-[var(--glass-border)] ">
 {payment.status !== 'PAID' && payment.paymentMethod === 'MIDTRANS' ? (
 <div className="text-center space-y-4">
 <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-amber-100 dark:bg-blue-900/30 text-[var(--accent)] dark:text-blue-400 mb-2">
 <CreditCard className="w-6 h-6" />
 </div>
 <p className="text-sm text-slate-600 dark:text-[var(--text-secondary)]">Selesaikan pembayaran Anda melalui layanan Midtrans.</p>
 <button
 ...
 placeholder="https://..."
 className="w-full bg-white dark:bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-all"
 />
 </div>
 <button
 type="submit"
 disabled={submittingProof}
 className="w-full py-3 rounded-xl bg-[var(--bg-primary)] hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-secondary)] text-white dark:text-[var(--text-primary)] text-sm font-semibold transition-all duration-300 hover:scale-[1.02] active:scale-95 shadow-lg disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center gap-2"
 >
 ...
 <div>
 <h3 className="text-lg font-bold text-[var(--text-primary)] ">Asesmen Medis</h3>
 <p className="text-xs text-[var(--text-secondary)]">Pernyataan ini wajib diisi demi keselamatan Anda.</p>
 </div>
 </div>
 ...
 className="w-full bg-[var(--bg-main)] dark:bg-[var(--bg-secondary)]/50 border border-[var(--glass-border)] rounded-xl p-4 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-all resize-none"
 placeholder="Sebutkan jika ada riwayat asma, jantung, alergi obat tertentu, dsb."
 />
 </div>

 <div className="p-4 rounded-xl bg-[var(--bg-main)] dark:bg-[var(--bg-secondary)]/50 border border-[var(--glass-border)] flex gap-3 items-start">
 <ShieldCheck className="w-5 h-5 text-[var(--accent)] shrink-0 mt-0.5" />
 <p className="text-xs text-slate-600 dark:text-[var(--text-secondary)] leading-relaxed">
 ...
 <button
 type="submit"
 className="w-full py-3 rounded-xl bg-[var(--bg-primary)] hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-secondary)] text-white dark:text-[var(--text-primary)] text-sm font-semibold transition-all duration-300 hover:scale-[1.02] active:scale-95 shadow-lg"
 >
 ...
 <h3 className="text-2xl font-black text-[var(--text-primary)] uppercase">{result.participantName}</h3>
 
 <div className="grid grid-cols-2 gap-4">
 <div className="bg-[var(--bg-main)] dark:bg-[var(--bg-secondary)]/50 border border-slate-100 rounded-xl p-5 text-left">
 <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">Gun Time</p>
 <p className="text-3xl font-display tracking-widest text-[var(--text-primary)] ">{result.gunTime}</p>
 </div>
 <div className="bg-[var(--bg-main)] dark:bg-[var(--bg-secondary)]/50 border border-slate-100 rounded-xl p-5 text-left">
 <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">Chip Time</p>
 <p className="text-3xl font-display tracking-widest text-[var(--accent)] dark:text-[var(--accent)]">{result.chipTime}</p>
 </div>
 </div>
 ...
 className="w-full py-3.5 rounded-xl bg-[var(--bg-primary)] hover:bg-[var(--bg-secondary)] dark:bg-gradient-to-r dark:from-amber-400 dark:to-amber-500 text-white dark:text-[var(--text-primary)] text-sm font-semibold transition-all duration-300 hover:scale-[1.02] active:scale-95 shadow-lg mt-4"
 >
 Lihat E-Sertifikat
 </button>
 )}
 ...
 <div className="py-4">
 <h3 className="text-lg font-bold text-[var(--text-primary)] mb-2">Hasil Belum Tersedia</h3>
 ...
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--bg-primary)]/40 backdrop-blur-sm animate-in fade-in duration-200">
 <div className="bg-white dark:bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-2xl p-6 max-w-sm w-full relative shadow-xl">
 <button
 onClick={() => setShowQrModal(false)}
 className="absolute top-4 right-4 p-2 text-[var(--text-secondary)] hover:text-slate-600 dark:hover:text-slate-300 hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-secondary)] rounded-lg transition-colors"
 >
 <X className="w-5 h-5" />
 </button>
 <div className="text-center mb-4">
 <h3 className="text-lg font-bold text-[var(--text-primary)] ">QR Code Check-In</h3>
 <p className="text-xs text-[var(--text-secondary)]">Tunjukkan QR ini kepada panitia</p>
 </div>
