    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 text-center max-w-2xl mx-auto">
          ...
          <h1 className="text-3xl font-black text-white uppercase tracking-tight">Hasil Lomba Resmi</h1>
          <p className="text-slate-400 text-xs mt-1">Cari waktu tempuh, peringkat kategori, dan status finisher peserta.</p>
        </div>

        {/* Controls */}
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl mb-8 flex flex-col md:flex-row gap-4 justify-between items-center">
          <div className="flex items-center gap-3 w-full md:w-auto">
            <label className="text-xs font-bold text-slate-400 uppercase shrink-0">Pilih Event:</label>
            <select
              value={selectedEventId}
              onChange={(e) => handleEventChange(e.target.value)}
              className="bg-slate-950 border border-slate-800 text-slate-200 text-xs font-bold rounded-xl px-4 py-2.5 focus:outline-none focus:border-orange-500 w-full md:w-72"
            >
        ...
        {/* Results Table */}
        {loading ? (
          <div className="bg-slate-900 rounded-2xl h-64 border border-slate-800 animate-pulse" />
        ) : filteredResults.length === 0 ? (
          <div className="p-16 text-center bg-slate-900/50 rounded-2xl border border-slate-800">
            <Award className="w-12 h-12 text-slate-600 mx-auto mb-4" />
            <h3 className="text-lg font-bold text-white">Belum Ada Hasil Resmi</h3>
            <p className="text-xs text-slate-400 mt-1">Hasil lomba belum dipublikasikan oleh komite penilai untuk filter ini.</p>
          </div>
        ) : (
                ...
                <tbody className="divide-y divide-slate-800/60 font-medium">
                  {filteredResults.map((r, idx) => (
                    <tr key={r.id} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="p-4 font-mono font-bold text-white">{r.bibNumber}</td>
                      <td className="p-4 font-bold text-white">{r.participantName}</td>
                      <td className="p-4 text-slate-400">{r.gender === 'MALE' ? 'Pria' : 'Wanita'}</td>
                      <td className="p-4 font-mono text-emerald-400 font-bold">{r.chipTime || '-'}</td>
                      <td className="p-4 font-mono text-slate-300">{r.gunTime || '-'}</td>
                      <td className="p-4">
