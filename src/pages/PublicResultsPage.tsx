    ...
    <div className="min-h-screen  text-slate-900 dark:text-slate-100 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 text-center max-w-2xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-yellow-500/10 border border-yellow-500/20 text--600 dark:text--400 text-xs font-bold uppercase tracking-wider mb-3">
          ...
          <p className="text-slate-500 dark:text-slate-500 dark:text--600 dark:text--400 text-xs mt-1">Cari waktu tempuh, peringkat kategori, dan status finisher peserta.</p>
        </div>
        ...
        <div className="bg-white dark:bg-blue-950/80 backdrop-blur-xl border border-slate-200 dark:border-white/[0.06] p-4 rounded-2xl mb-8 flex flex-col md:flex-row gap-4 justify-between items-center">
          <div className="flex items-center gap-3 w-full md:w-auto">
            <label className="text-xs font-bold text-slate-500 dark:text-slate-500 dark:text--600 dark:text--400 uppercase shrink-0">Pilih Event:</label>
            <select
          ...
          <div className="relative w-full md:w-80">
            <Search className="w-4 h-4 text-slate-500 dark:text-slate-500 dark:text--600 dark:text--400 absolute left-3.5 top-3.5" />
            <input
              type="text"
              value={search}
        ...
        {loading ? (
          ...
          <div className="p-16 text-center bg-white dark:bg-blue-950/50 rounded-2xl border border-slate-200 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-500 dark:text--600 dark:text--400 mt-1">Hasil lomba belum dipublikasikan oleh komite penilai untuk filter ini.</p>
          </div>
        ) : (
          <div className="bg-white dark:bg-blue-950/80 backdrop-blur-xl border border-slate-200 dark:border-white/[0.06] rounded-2xl overflow-hidden shadow-xl">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className=" border-b border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-500 dark:text--600 dark:text--400 font-bold uppercase tracking-wider">
                ...
                <tbody className="divide-y divide-slate-800/60 font-medium">
                  {filteredResults.map((r, idx) => (
                    <tr key={r.id} className="hover:bg-slate-100 dark:bg-slate-800/50 transition-colors">
                      <td className="p-4 font-black text--600 dark:text--400">
                        {r.categoryRank <= 3 ? (
                      ...
                      <td className="p-4 text-slate-500 dark:text-slate-500 dark:text--600 dark:text--400">{r.gender === 'MALE' ? 'Pria' : 'Wanita'}</td>
                      <td className="p-4 font-mono text--600 dark:text--400 font-bold">{r.chipTime || '-'}</td>
                      ...
                      <td className="p-4">
                        <span className={`px-2.5 py-1 rounded text-[10px] font-black uppercase ${
                          r.status === 'FINISH' ? 'bg-emerald-950 text--600 dark:text--400 border border-emerald-800/50' : 'bg-red-950 text--600 dark:text--400'
                        }`}>
                          {r.status}
                        </span>
                      </td>
