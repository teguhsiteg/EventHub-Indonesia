...
export const ResetPasswordPage: React.FC = () => {
 ...
 <div className="w-full max-w-md bg-white dark:bg-[#020617] rounded-3xl shadow-xl border border-[var(--glass-border)] dark:border-white/[0.08] p-8 relative z-10">
 ...
 Masukkan kata sandi baru untuk akun <strong className="text-slate-700 ">{email}</strong>
 </p>
 )}
 </div>
 ...
 <p className="text-sm text-[var(--text-secondary)]">Memverifikasi tautan...</p>
 </div>
 ) : error ? (
 ...
 <Link
 to="/login"
 className="inline-flex items-center justify-center w-full py-3 rounded-xl bg-[var(--bg-secondary)] hover:bg-slate-200 /[0.04] dark:hover:bg-white/[0.08] text-slate-700 text-sm font-semibold transition-all"
 >
 Kembali ke Login
 </Link>
 </div>
 ...
 <h2 className="text-xl font-bold text-[var(--text-primary)] ">Kata Sandi Berhasil Diubah!</h2>
 <p className="text-sm text-[var(--text-secondary)] dark:text-[var(--text-secondary)]">
 ...
 required
 className="w-full pl-10 pr-10 py-3 rounded-xl bg-slate-50 /[0.03] border border-[var(--glass-border)] dark:border-white/[0.08] text-[var(--text-primary)] placeholder:text-[var(--text-secondary)] text-sm focus:outline-none focus:border-red-500/50 focus:ring-1 focus:ring-red-500/20 transition-all"
 />
 <button
 type="button"
 onClick={() => setShowPassword(!showPassword)}
 className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[var(--text-secondary)] hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
 >
